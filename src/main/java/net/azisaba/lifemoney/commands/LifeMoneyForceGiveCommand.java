...
public class LifeMoneyForceGiveCommand implements TabExecutor {
    ...
    public @Nullable List<String> onTabComplete(@NotNull CommandSender commandSender, @NotNull Command command, @NotNull String s, @NotNull String[] strings) {
        ...
        if (strings.length == 1) {
            ...
            return Bukkit.getOnlinePlayers().stream().map(Player::getName).filter(name -> name.toLowerCase().startsWith(strings[0].toLowerCase())).toList();
        ...
        if (strings.length == 3) {
            ...
            return Arrays.stream(Moneys.values()).map(Moneys::name).filter(name -> name.toLowerCase().startsWith(strings[2].toLowerCase())).toList();
        }
